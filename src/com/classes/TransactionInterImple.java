...
public boolean queryBooker(String bookerID, int serverNO) {
	...
	if(connection==null){
		...
		String queryString = "select * from booker"+serverNO + " where bookerID = ?";
		PreparedStatement statement;
		try{
			statement = connection.prepareStatement(queryString);
...
public boolean insertBooker(String bookerID, String bookerName,
	...
	// TODO Auto-generated method stub
	 if(connection==null){
		 ...
		 if(this.queryBooker(bookerID, serverNO)){
			 return false;
		 }else{
		 int flag=0;
		 String updateString ="insert into booker"+serverNO + " values(?, ?, ?, ?, ?)";
		 PreparedStatement statement;
		 try{
			 statement = connection.prepareStatement(updateString);
			 statement.setString(1, bookerID);
...
public String bookingRoom(String checkinDate, String checkoutDate, String hotelID, String transactionID,
	...
	if(connection==null){
		...
		try{
			...
			if(flag1==1&&flag2==1){
				this.insertTransaction(transactionID, roomID, new Date(System.currentTimeMillis()).toString(), bookerID, creditNO, hotelID, serverNO);
...
public static void main(String[] args) {
	  //TransactionInterImple a = new TransactionInterImple();
      //boolean c=a.insertBooker("122222", "dfasf", "dfadsf", "dfadsf", "dfasdf", 1);
